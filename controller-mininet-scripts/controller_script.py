...
class Switch(EventMixin):
    ...
    def install(self, port, match, modify = False, buf = -1, idle_timeout = 600, hard_timeout = 600):
        log.info("***Install Called***  =>  added entry to flow table")
        msg = of.ofp_flow_mod()
        msg.match = match
        if modify:
        ...
        output.write("<h5>Flow Entry Added:</h5>")
        output.write("<p>"+ str(msg) +"</p>")
        self.connection.send(msg)
...
class Controller(EventMixin):
    ...
    def _install_reactive_path(self, event, out_dpid, final_out_port, packet):
        ...
        log.info("*** reactive path called ")
        output = open('results'+"/data.html",'a')
        in_name = self.t.id_gen(dpid = event.dpid).name_str()
        out_name = self.t.id_gen(dpid = out_dpid).name_str()

        print "in name: ", in_name, "dpid: ", event.dpid
        print "out name: ", out_name, "dpid: ", out_dpid
        # hash_ = self._ecmp_hash(packet)
        route = self.r.get_route(in_name, out_name)
        print "ROUTE: ", route
        # HTML Code
        output.write("<h3>Reactive Path</h3>")
        output.write("<b>In Name: </b>"+ in_name+"&nbsp;&nbsp;<b>Out Name: </b>"+ out_name+ "</br>")
        output.write("<p><b>Route: </b>"+ str(route) +"</p>")
        match = of.ofp_match.from_packet(packet)
        for i, node in enumerate(route):
            node_dpid = self.t.id_gen(name = node).dpid
            if i < len(route) - 1:
                next_node = route[i + 1]
                print "node : ", node, "next node: ", next_node
                out_port, next_in_port = self.t.port(node, next_node)
            else:
                out_port = final_out_port
            self.switches[node_dpid].install(out_port, match, idle_timeout = 600)
    ...
    def _handle_PacketIn(self, event):
        ...
        output = open('results'+"/data.html",'a')
        # print "packet: ", packet, "dpid: ", dpid, "in port: ", in_port, "topo: ", t
        ...
        # print "SRC MAC: ", srcMac, "DEST MAC: ", destMac
        ...
        if str(destMac) in self.macTable:
            ...
            # output.write("Source MAC: " + str(srcMac) + "\nDestination MAC: " + str(destMac) + "\n")
            # output.write("Updated MAC Table: " + str(self.macTable) + "\n\n")

            log.info("*****Packet in event called*****")
            print "SRC MAC : ", srcMac
            print "DEST MAC : ", destMac
            # HTML Code for output
            output.write("<h3>Packet In Event</h3>")
            output.write("<b>Source MAC: </b>"+ str(srcMac) +"</br>")
            output.write("<b>Destination MAC: </b>"+ str(destMac) +"</br>")
            log.info("**Updated Mac Table**")
            print self.macTable
            output.write("<i>Updated Mac Table</i></br><span>"+ str(self.macTable) +"</span>")
